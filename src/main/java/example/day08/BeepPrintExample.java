...
public class BeepPrintExample {
    ...
    /*
        ...
        스레드 : 코드의 실행 흐름
            - 스레드 마다 스택 할당
            - 스레드끼리 자원 공유 안됨
            - 하나의 스레드가 오류/예외 발생시 전체 스레드가 오류/예외 발생
            -
        멀티스레드 : 여러개의 코드 실행 흐름
            ...
            작업요청
                멀티 : 안됨X
                여러개 스레드들의 작업 순서는 하나씩 처리( 컴퓨터는 빠르기 때문에 동시처리 처럼 보인다 ) -> 순차 처리
                작업순서 : 운영체제가 스케줄링
    ...
    public static void main(String[] args) {
        ...
        for ( int i = 1 ; i <= 5 ; i++) {   // main스레드가 for 실행
            toolkit.beep(); // 띠링 소리 발생시키는 함수
            // 띵소리가 for문이 5번 회전하는것보다 느리기 때문에 띵소리 한번남
                // Thread.sleep( 밀리초 ) : 밀리초(1/1000초) 만큼 일시정지
                    // * 일반예외처리 : 혹시나 일시정지 도중에 다른쪽 스레드가 예외 발생시킬수 있으니까
