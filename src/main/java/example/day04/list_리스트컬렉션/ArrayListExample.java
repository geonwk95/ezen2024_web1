package example.day04.list_리스트컬렉션;
    ...
    public static void main(String[] args) {
        ...
        // 1. List
            ...
            // - ArrayList , Vector , LinkedList 클래스
                // 동기화 : 여러 스레드가 하나의 함수를 동시에 호출했을때 호출순서대로 해당 함수를 끝날때까지 점유/대기 상태
                    // 쉽게 생각해서 1인실 공부방 ~~
                // ArrayList : [배열구조] 동기화 지원 X , 주로 단일스레드 에서 사용
                // Vector   : [배열구조] 동기화 지원 O , 주로 멀티스레드 에서 사용
                // LinkedList : [연결구조] 동기화 지원 X , 별도의 추가 라이브러리 를 이용하면 동기화가능
        ...
        // 리스트객체명.add( 인덱스 , 객체 ) : 주어진 객체를 리스트내 주어진 인덱스에 추가
        ...
        System.out.println("2번 인덱스 의 객체 : board1 = " + board1);
        System.out.println();

        // 5. 리스트내 저장된 특정 인덱스의 객체 존재 여부
        boolean result = list.contains( board1 );
        ...
        System.out.println("향샹된 for문 리스트 순회 : ");
        for ( Board b : list ){
